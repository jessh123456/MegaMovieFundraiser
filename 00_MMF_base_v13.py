...
Create the summary frame - below the profit/pricing calculations created in v12
...
new ticket profit calculation under snack profit calculation() in the main
routine.
In the main while loop, after calculating the ticket price, calculate ticket
profit and add to running total
Removed the whole "while_getting_snacks" loop - deleted everything between
...
Took out references to getting_snacks - if option == "X" - and replaced it with
a break
...
while name != "X" and ticket_count != 5:
    ...
    if name == "X":
        ...
        # print(snack_order)
        for item in snacks_order:  # the item only has 2 parts - number and snack
        ...
        if len(snacks_order) > 0:  # If there is something in it, print each item
